                        ...
                        if (distX < 1.0 && distY < 1.0) {
                            ...
                            if (overlapY > 0.001 && overlapY <= overlapX) {
                                ...
                                if (this.vy > 0 && this.y < blockCenterY) {
                                    // Fall damage - scaled for max velocity ~0.367
                                    if (!collided && this.vy > 0.2) {
                                        const fallDamage = Math.pow(this.vy, 2) * 200;
                                        this.hull -= fallDamage;
                                        collided = true;

                                        // Visual feedback for fall damage
                                        if (game) {
                                            game.spawnParticles(this.x, this.y, '#ff4400', Math.min(25, Math.floor(fallDamage)));
                                        }
                                    }
                                    this.vy = 0;
